...
|---|---|---|---|
| 1 | **Modo escuro (D-44)** — **3 de 7 frentes feitas.** Restantes na ordem: **7 → 3 → 5 → 2** | `Header.tsx`, `index.css`, `three/*` | 🔨 em andamento — ✅ frente 4 (`d17ee23`), frente 1, frente 6 (`dd32ee9`) |
| 2 | **Continentes sólidos e partículas brancas no escuro (D-58)** | `public/images/world-dots.png` (regerar sólido), `three/Backdrop.tsx` | a fazer — a parte das partículas é **dentro da frente 2** |

**⚠ Dependência entre frentes:** o `apply()` do tint só roda em evento de
scroll, então **o botão da frente 7 tem de forçar um reapply ao trocar de
tema** — senão clicar no sol não repinta até a pessoa rolar. Ver fim do D-57.

**Aberto para o olho do Raul, não é código:** no globo novo o pin é maior que a
Irlanda e cobre a ilha (ver fim do D-56). Se incomodar, é reduzir cabeça e pé —
