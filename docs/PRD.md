...
| **Status** | Aprovado — em execução (**Passo 9 fechado**; Passo 10 só com a performance mobile do RNF-01 em aberto) |
...
| 7 | ⚠ **Performance mobile (RNF-01)** — 68 contra a meta de 80. **Causa medida, conserto não proposto** | `index.html`, empacotamento | **a fazer** — ver o bloco abaixo |
| último | **Site bilíngue (D-61)** — depois da revisão do inglês | textos, roteamento, header | a fazer |

### 📊 Lighthouse em `raulrodrigues.dev` (RNF-01) — 07/09

Sete das oito notas passam. **Metas: performance ≥ 80, o resto ≥ 90.**
...
Mobile: FCP 4,6 s · LCP 4,9 s · TBT 170 ms · CLS **0**.
Desktop: FCP 1,0 s · LCP 1,1 s · TBT 20 ms · CLS **0**.

**Método, porque muda a confiança do número:** o `pagespeed.web.dev` que o
RNF-01 cita recusou as duas chamadas com **429, cota diária esgotada** da chave
anônima. As notas acima saem do **Lighthouse 12.8.2 rodando local** contra o
Chrome 152 instalado, apontado para a URL de produção — mesmo motor e mesmas
auditorias, com o estrangulamento simulado padrão. Refazer pelo PSI quando a
cota voltar deve dar números próximos, não idênticos.

**As duas causas da nota mobile, medidas. Nenhum conserto proposto ainda**, que
era o combinado — e as duas têm a mesma forma: trabalho que o mobile paga e
não usa.

1. **A folha do Google Fonts bloqueia a renderização por 825 ms.** É o maior
   item do caminho crítico, contra 155 ms do CSS próprio. Antes de qualquer
   texto aparecer, o navegador resolve DNS, abre TLS e busca um CSS de 1,2 KB
   em `fonts.googleapis.com`, e só então descobre os dois `woff2` em
   `fonts.gstatic.com`. Os `preconnect` que já existem no `index.html` ajudam
   na conexão e **não resolvem o bloqueio**, porque o problema é a folha ser
   render-blocking, não a latência de abrir a conexão.

2. **237 KB de bibliotecas 3D baixadas no celular, que o celular nunca monta.**
   O `useCanRender3D` recusa a cena abaixo de 640 px, e recusa certo — o LCP
   da página mobile é o `memoji.webp`, ou seja o fallback estático entrou.
   **Mas o HTML publicado traz `<link rel="modulepreload">` para
   `three-*.js` (183 KB) e `r3f-*.js` (54 KB)**, então o navegador baixa os dois
   de qualquer jeito, antes de o React sequer decidir. Medido pelo Lighthouse:
   **202 KiB de JS não usado numa página de 372 KiB** — 123 KB dos 168 do
   `three`, 47 dos 54 do `r3f` — e o `r3f` ainda custa **466 ms de avaliação
   de script** numa tela onde nada 3D existe.

   ⚠ **Isto também contradiz o RNF-02 na letra.** Ele exige `three` "em chunk
   próprio **carregado sob demanda**". O chunk próprio existe; a demanda não é
   respeitada, porque a dica de preload no HTML não sabe da regra de 640 px que
   mora no React. O bundle passa no teto de 150 KB e falha no "sob demanda", e
   a medição do RNF-02 (`npm run build`) nunca pegaria isso: ela olha o tamanho
   dos chunks, não quem os baixa.

**A fila anterior tinha esvaziado.** Os três itens que estavam aqui saíram na
...
**Onde a próxima sessão pega o trabalho.** Sessão de 07/09 encerrada. **Nada
