...
|---|---|---|---|
| 1 | **Memoji de volta ao centro (D-43)** — está 335 px à direita | `sections/Hero.tsx`, `three/HeroVisual.tsx` | a fazer |
...
| 6 | Links e botões magnéticos (M-27) | CTAs do Statement, copiar e-mail, baixar CV | a fazer |
...
**Dependência entre itens 2 e 4:** aplicar o reveal aos `h2` antes de corrigir o
espaço do `WordReveal` transforma um defeito num `h1` em seis — `SelectedWork`,
`HowIwork` e os demais. O item 2 vem antes, ou no mesmo commit.
...
| M-26 | Títulos de seção revelando palavra a palavra, reusando o `WordReveal` com stagger menor que o do hero | todas as seções | extra | 🔨 em andamento — **depende da correção do `WordReveal`** |
| M-27 | Links e botões magnéticos: o elemento se desloca até ~8 px na direção do cursor a ~60 px e volta ao sair | CTAs do Statement, copiar e-mail, baixar CV | extra | 🔨 em andamento (promovido do `BACKLOG`) |
