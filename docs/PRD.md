...
| **Status** | Aprovado — em execução (**Passos 9 e 10 fechados em código**; falta só a revisão do inglês, que é humana) |
...
| 12 | ⚠ **RNF-02 continua descumprido** — `three` e `r3f` baixam no celular por **import estático**, não por preload. Conserto é **desenho de grupo de chunk**, e é decisão nova | `vite.config.ts` | **a fazer** — precisa de decisão do Cowork antes do código |
| último | **Site bilíngue (D-61)** — depois da revisão do inglês | textos, roteamento, header | a fazer |

### 📊 Lighthouse em `raulrodrigues.dev` (RNF-01) — 08/09, depois da D-62

**Aceite cumprido: mobile 82, acima da meta de 80, e nenhuma das outras sete
notas regrediu.**

| | Performance | Acessibilidade | Best Practices | SEO |
|---|---|---|---|---|
| **Desktop** | **98** ✅ (era 95) | **100** ✅ | **100** ✅ | **100** ✅ |
| **Mobile** | **82** ✅ (era 68) | **100** ✅ | **100** ✅ | **100** ✅ |

Mobile: FCP **2,7 s** (era 4,6) · LCP **3,7 s** (era 4,9) · TBT **80 ms** (era
170) · CLS 0.
Desktop: FCP **0,5 s** (era 1,0) · LCP **0,8 s** (era 1,1) · CLS 0.

**Quem entregou os 14 pontos foi a D-62 (a) sozinha**, e vale registrar porque a
própria decisão previu o caso: *"se um dos dois estourar o custo, o outro
entrega sozinho"*. O bloqueio de renderização caiu de **825 ms do Google Fonts
mais 155 ms do CSS próprio** para **150 ms do CSS próprio e mais nada** — não
sobrou nenhum recurso bloqueante de terceiro no caminho crítico.

### ⚠ A D-62 (b) foi implementada e **não** atingiu o objetivo dela

O `modulePreload.resolveDependencies` está no `vite.config.ts` e as duas dicas
sumiram do HTML construído. **Mas a medição nova do RNF-02 reprova**, e é ela
que existe justamente para isso — a antiga (`npm run build`) diria que está tudo
certo.

| RNF-02, carga real a 412 px | Resultado |
|---|---|
| `three` e `r3f` **não** aparecem na rede | ❌ **falha** — 237,5 KB baixados |
| JS não usado | 202 KiB de 372 KiB — **igual a 07/09** |

**A premissa da D-62 (b) estava incompleta, e agora a causa está medida.** A
dica de `modulepreload` nunca foi a amarra: ela apenas **antecipava** um
download que aconteceria de qualquer jeito. O chunk de entrada **importa
estaticamente** o chunk `r3f`:

```
import{a as n,i as r,o as i}from"./r3f-4tHL9_Td.js"
```

e o `r3f`, por sua vez, importa estaticamente o `three`. Import estático o
navegador **tem** de resolver antes de executar o módulo — nenhuma dica de
preload muda isso, e tirar a dica só troca a prioridade do download.

**Por que a entrada depende do `r3f`, se nenhum módulo do grafo ansioso importa
`@react-three`:** o chunk chamado `r3f` **não contém só `@react-three`**.
Conferido dentro do próprio arquivo construído, ele carrega também **`react-dom`
e `scheduler`**. O `main.tsx` precisa de `react-dom` para o `createRoot`, então
a página não tem como iniciar sem baixar o `r3f` — e o `r3f` arrasta o `three`
junto. O agrupador varreu a família do React para dentro do grupo do
`@react-three`.

**O `React.lazy` e o `useCanRender3D` estão certos e não foram tocados.** A
412 px a cena realmente nunca monta, e o LCP do celular é o `memoji.webp`. O que
baixa não é a cena: é a biblioteca, por uma aresta de empacotamento.

**Conserto apontado e não feito, de propósito.** Ele é diferente do que a D-62
autorizou — mexeria no desenho dos grupos de `codeSplitting`, que muda fronteira
de chunk e cache do site inteiro, e não em filtrar uma lista de preload. Não é
coisa para improvisar num deploy de aceite. O caminho provável é dar a
`react`/`react-dom`/`scheduler` um **grupo próprio**, para a entrada deixar de
depender do chunk do 3D. **Isso é decisão nova, não estado**, então fica aqui
apontado para o Cowork escrever.

**Fica valendo o que a medição nova provou de si mesma:** ela pegou um defeito
que passou pelo `npm run build`, pelo lint, pelo TypeScript e pelo próprio
Lighthouse — que deu 82 com os 237 KB ainda sendo baixados. Nota boa não é prova
de contrato cumprido.
...
**Onde a próxima sessão pega o trabalho — 08/09.** Nada ficou local: os três
commits desta sessão estão em produção, em `72b39fd`.
